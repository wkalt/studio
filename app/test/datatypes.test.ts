...
import { createRosDatatypesFromFrame, FieldType, inferDatatypes } from "./datatypes";

const unknown: FieldType = { type: "unknown", isArray: false };
...
describe("createRosDatatypesFromFrame", () => {
  ...
  it("handles arrays", () => {
    ...
    expect(createRosDatatypesFromFrame(topics, frame)).toEqual({
      foo: {
        fields: [
          { name: "thingsWithArrays", isComplex: true, type: "test_msgs/t1/auto_0", isArray: true },
        ],
      },
      "test_msgs/t1/auto_0": {
  ...
  it("does not classify any old metadata field as JSON", () => {
    ...
    expect(createRosDatatypesFromFrame(topics, frame)).toEqual({
      "fake_msgs/HasMetadata": {
        fields: [
          { name: "metadata", type: "test_msgs/t1/auto_0", isArray: false, isComplex: true },
        ],
      },
